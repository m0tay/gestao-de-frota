  ...
  <Modal :show="props.show" @close="$emit('close')">
    <section class="space-y-6 p-8 w-full" @contextmenu.prevent>
      ...
      <div class="mt-6 max-w-full">
        ...
        <Textarea disabled class="w-full" id="description" v-model="form.description" :placeholder="form.description"/>
        <InputError :message="form.errors.description"/>
      </div>

      <div class="mt-6 max-w-full">
        <InputLabel value="Pretexo do reagendamento" for="reason_for_status_change"/>
      ...
      <div class="mt-6 flex flex-col gap-y-4 justify-end gap-x-4 sm:flex-row">
        ...
        <Button variant="destructive" class="hover:animate-pulse"
                :class="{ 'opacity-25': formCancel.processing }"
                :disabled="formCancel.processing"
                @click="handleCancel"
        >
          Cancelar Requsição
        </Button>
